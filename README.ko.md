...
- **gitmail 아웃리치** — GitHub에서 비슷한 레포 찾아 별표 누른 사람들에게 짧은 개인 메일. 한 번에 최대 1,500명 (GraphQL 일괄 프로필 조회 + REST PushEvent 폴백으로 GitHub의 두 5,000/hr 버킷을 함께 활용), 원클릭 구독해지 자동.
- **트위터 답글 후보 찾기** — `/twitter-reply`로 "이거 만들었는데 한번 보실래요?"라고 답글 달기 자연스러운 최근 X 트윗을 검색해서 대시보드 `/twitter-reply` 페이지에 카드(본문/작성자/링크/인게이지먼트)로 띄워줍니다. 답글 발송은 트윗별 명시적 확인 후에만 진행됩니다.

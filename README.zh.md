...
把您的项目描述输入进来，viralman 会为您起草 Twitter/X 推文、Reddit 帖子，以及发给在 GitHub 上 star 过同类项目的开发者的冷邮件。无论是开源项目还是个人副业都可以使用。是否发送由您确认。
...
- **gitmail 外联** —— 在 GitHub 找到与您项目最相似的仓库,遍历其 stargazer,为每位发送一封简短个性化邮件。每次最多 1,500 名收件人(GraphQL 批量获取 profile + REST PushEvent 兜底,同时利用 GitHub 两个独立的 5,000/hr 配额),自带一键退订链接。
- **Twitter 回复候选** —— `/twitter-reply` 搜索最近的 X 推文,找出适合用「我做了这个,要不要看一下?」回复的对象,并将候选推送到 dashboard `/twitter-reply` 页面,以卡片(正文 / 作者 / 链接 / 互动数)形式展示。回复需要逐条显式确认。
- **AI 痕迹 sniffer** —— 约 30 条规则扫描每份草稿:陈词滥调、em-dash 滥用、平衡三段式、缺锚点。最多 3 轮重写,仍未通过则拒绝自动发布。
- **OAuth 或手动** —— 您可以在 dashboard 登录 X / Reddit / LinkedIn,或粘贴 token。密码绝不进入 LLM 上下文。
- **多 LLM** —— 您可以在 Claude / OpenAI / Gemini 中任选一个,系统会按已保存的 API key 自动识别。

## 适用场景

- **v1.0 发布** —— 写明上线了什么,即可立刻获得 r/programming 的 Reddit 帖子、X 串文、LinkedIn 公告,以及 star 过同类工具的开发者名单。
- **副业项目宣告** —— 无需为三个平台分别撰写。一次输入 → 多渠道发布。
- **不知道该在哪儿发布** —— viralman 用项目关键词为您抓取并推荐合适的子版块、hashtag 和最近可以评论的帖子。
- **重新激活同类工具的老 stargazer** —— gitmail 用公开 profile 和 commit 邮箱建立名单,开场白会提到对方点过星的仓库以实现个性化。
- **躲开 AI 味** —— 大多数「AI 社交发帖工具」一眼就被识破。Sniffer 是 viralman 的核心差异点。

## 安装

我们提供三条安装路径。一般推荐路径 1;若您不使用 Claude Code,请选路径 2;CI / 自动化场景请选路径 3。

> 您可以按照下面的手动设置步骤完成配置,但更推荐安装 Claude Code 插件(路径 1),让智能体直接帮您完成所有设置。

### 路径 1 —— Claude Code 插件(推荐)

适合大多数 Claude Code 用户的 marketplace/plugin 安装方式。下面两行是 Claude Code 斜杠命令,请**逐行输入**(若您一次粘贴两行会失败):
...
如果您已经将仓库克隆到本地,URL 可换成 `./`:

```
/plugin marketplace add ./
```

安装完成后,无需记任何命令 —— 您可以直接用自然语言告诉智能体,例如 `"打开面板"`、`"配置 viralman"`、`"给 async-profiler 的 starrer 发邮件"`,智能体会自动调用 `/dashboard`、`/viralman-setup`、`/gitmail`、`/viral`。发送前会依次为您确认 (1) 语言 (2) 主题风格 (3) 最终确认。

### 路径 2 —— pipx 安装(无需 Claude Code)

如果您只想使用本地 Dashboard + 纯 CLI:
...
`pipx` 会创建独立 venv 并将 `viralman` 命令注入您的 `$PATH`。如您已有 venv,使用 `pip install git+...` 同样可行。Dashboard 4 个标签页(Twitter / Reddit / Gitmail / Setup)涵盖全部操作;斜杠命令需要 Claude Code 才能使用。

### 路径 3 —— 克隆后直接运行(CI / headless / 自动化)

如果您需要在脚本或 CI 流水线中以显式参数运行:
...
完整的 gitmail / viral 参数请参见下文 [使用](#使用示例) 章节。

## 使用示例

### Block A —— 在 Claude Code 中使用自然语言(路径 1 用户)

请直接告诉智能体即可 —— 它会自动选择正确的斜杠命令:
...
智能体会自动触发 `/gitmail`、`/viral`、`/dashboard` 或 `/viralman-setup`。
发送前会为您依次确认 (1) 语言 (2) 主题风格 (3) 最终确认。
...
```bash
/viralman-setup            # 选择渠道 (gitmail / twitter / reddit / linkedin) 并配置
/viralman-setup gitmail    # 直接进入 gitmail 分支
/viralman-setup --check    # 查看您已保存的 key
```
...
无需 API key 也可使用:若您已安装 **Claude Code**,viralman 会自动检测本地 `claude` 二进制并通过它进行 LLM 调用(Claude Max plan 配额)。请在 Dashboard 中选择 `claude (Max via CLI)`。

密码不会进入 LLM 上下文 —— `read -s` 会直接写入 `~/.viralman/.env`(`chmod 600`)。
...
1. **项目** —— 请输入名称、URL、一句话定位、详细描述。
2. **生成** —— 请选择频道(X / Reddit / Gitmail),系统会为您生成草稿。
3. **目标** —— 请选择子版块、hashtag、可评论的帖子、收件人名单。系统会为您自动建议。
4. **发送** —— 请确认后查看实时进度。
...
- `--keywords k1,k2` —— 用您指定的关键词替代自动分析结果。
- `--topics t1,t2` —— topics 覆盖。

每封邮件都会自带一键退订链接和 `List-Unsubscribe` 头。SMTP 默认 30 封/分钟(您可通过 `SMTP_RATE_PER_MIN` 调整)。

## 「看起来不像 AI」是怎么做到的

`ai-tell-sniffer` 会对每份草稿运行检查:禁用词(「delve」、「leverage」、「let's dive in」、「supercharge」等 20 余个),每 60 字超过 1 个 em-dash,平衡式三段列举,结尾说教,hashtag 堆砌,没有具体锚点(数字 / 名称 / 时间 / 自我承认)的泛泛而谈。最多 3 轮重写,仍标红则会拒绝自动发布。

韩语输出同样会检测 12 种模式(활용하여 / 결론적으로 / 「X 아니라 Y」等),并执行说教检测和 em-dash 密度分析。
...
v0.3.0 —— 5 步交互式 gitmail 流程 + `/viralman-setup` 统一凭证入口 + `--tone` / `--emphasis` / `--seed-repos` 参数。本地 dashboard 和 v0.1.0 的 `/viral` 流程保持不变。

## 贡献

请参见 [`CONTRIBUTING.md`](CONTRIBUTING.md) 和 [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md)。安全问题请通过 [`SECURITY.md`](SECURITY.md) 联系我们。

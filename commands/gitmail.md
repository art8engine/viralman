...
allowed-tools: Read, Bash(viralman:*), Bash(grep:*), Bash(tail:*), Bash(head:*), Bash(wc:*)
...
1. **Pre-flight** — `viralman save-creds --show-keys` to verify `GITHUB_TOKEN` + one LLM key + (for real send) the SMTP cred set. If anything is missing, route to `/viralman-setup gitmail` and stop.
...
3. **Step 2 — Batch question (AskUserQuestion, exactly once)** — surface all four questions in one call. Do **not** call `viralman gitmail recipients` or `send-from-recipients` until the answers are in.
...
4. **Step 3 — Collection** — run `viralman gitmail recipients ...`. Slice the recipients array off the tail of the JSONL stream and save to `/tmp/gitmail_recipients_clean.json`. Print at most 8 in the preview.
...
- For live progress, use `viralman gitmail-watch --auto` (or `--once` for a single statusLine print).

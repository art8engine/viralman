...
allowed-tools: Read, Bash(.venv/bin/python:*), Bash(./scripts/twitter_reply.py:*), Bash(./scripts/save_creds.py:*), Bash(./bin/viralman:*), Bash(grep:*), Bash(tail:*), Bash(head:*), Bash(open:*)
...
1. **Pre-flight** — `./scripts/save_creds.py --show-keys` to verify `TWITTER_OAUTH2_BEARER` (search + reply scope) and one LLM key (or Claude Code CLI). If missing, route to `/viralman-setup twitter` and stop.
...
4. **Step 3 — Scrape** — `.venv/bin/python ./scripts/twitter_reply.py find ... --out /tmp/twitter_candidates.json`. Summarize: count + top 3 with handle + first 80 chars.
...
6. **Step 5 — Per-candidate reply** (Scrape+reply only) — compose, sniff, show, wait for "send" / "발송", then `./scripts/twitter_reply.py reply --tweet-id ... --body -`.
...
- Never call `twitter_reply.py reply` without an explicit per-candidate "send" / "발송" from the user.

...
Three pages: /twitter, /reddit, /gitmail. Header navigates between them.
Login is OAuth-first with a manual-token fallback. The page never sees raw
secrets — they go straight to ~/.viralman/.env via save_creds.

This module exposes `create_app()` so scripts/dashboard.py can spin it up.
"""
...
def create_app(*, secret_key: str | None = None) -> Flask:
    ...
    @app.route("/")
    def index():
        return redirect(url_for("page_twitter"))
    ...
    # Lightweight unsubscribe receipt — gitmail emails point here.

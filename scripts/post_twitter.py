...
def post_via_oauth2(creds: dict, body: str) -> str | None:
    """Post a thread via the v2 API using the OAuth2 user-context bearer.
    Returns the URL of the root tweet, or None if creds are missing or any
    leg of the post fails (caller falls back to compose URL)."""
    if not creds.get("TWITTER_OAUTH2_BEARER"):
        return None
    ...
    for tweet in parts:
        try:
            tid = twitter_v2.post_tweet(creds, text=tweet,
                                          in_reply_to_tweet_id=parent_id)
        except twitter_v2.TwitterAuthError as e:
            print(f"WARN: oauth2 auth failed: {e}", file=sys.stderr)
            return None
